...
// create  list - for now in V3 api only movie lists exists
  void createMovieList({required String name, String? description}) async {
    ...
    Get.showSnackbar(
      GetBar(
        title: 'Collections',
        message: '$name collecciion creada ...',
    ...
    await Future.wait([
      _service.addMovieToList(listId: listId, movieId: movieId).then((value) {
        if (value['success'] == true) {
          ...
          Get.showSnackbar(
            GetBar(
              message: 'Si, exitosamente agregada la peli: $listTitle',
              isDismissible: true,
              duration: const Duration(milliseconds: 1600),
              dismissDirection: SnackDismissDirection.HORIZONTAL,
              snackStyle: SnackStyle.GROUNDED,
  ...
  void removeMovieFromlist(
    ...
    await _service
      ...
      if (value['success'] == true) {
        Get.showSnackbar(
          GetBar(
            message: 'Vientos!!!! se ha agregado la peli: $listTitle',
            isDismissible: true,
            duration: const Duration(milliseconds: 1600),
            dismissDirection: SnackDismissDirection.HORIZONTAL,
            snackStyle: SnackStyle.GROUNDED,
            ...
            mainButton: TextButton(
              ...
              child: const Text('Ver como quedo...;)'),
            ),
          ),
        );
      }
